...
describe('CsRiskEngine', () => {
  ...
  it('should flag declining engagement and a stalled pipeline for an inactive account', async () => {
    ...
    mockOrganization({
      ...
      interactions: [
        { createdAt: oldDate }, { createdAt: oldDate }, { createdAt: oldDate },
      ],
    });
    ...
    expect(risks.map((r: any) => r.riskType)).toEqual(
      expect.arrayContaining(['declining_engagement', 'no_active_pipeline', 'high_loss_concentration'])
    );
  });
  ...
  it('should return no risks for a healthy organization', async () => {
    mockOrganization({
      ...
      interactions: [
        ...
        { createdAt: new Date() }, { createdAt: new Date() }, { createdAt: new Date() },
      ],
    });

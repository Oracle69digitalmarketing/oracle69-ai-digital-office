...
describe('CsHealthEngine', () => {
  ...
  it('should score a healthy organization and publish an event', async () => {
    mockOrganization({
      ...
      opportunities: [
        { stage: 'won' }, { stage: 'won' },
        { stage: 'negotiation' }, { stage: 'discovery' },
      ],
      interactions: [
    ...
    expect(prisma.crmOrganization.update).toHaveBeenCalledWith({
      where: { id: 'org-123' },
      data: expect.objectContaining({ healthScore: result.score, lastHealthUpdate: expect.any(Date) }),
    });
  ...
  it('should flag deterioration when the new score is lower than the previous score', async () => {
    mockOrganization({
      ...
      interactions: [{ createdAt: new Date() }, { createdAt: new Date() }, { createdAt: new Date() }],
    });

...
describe('CsSuccessPlanService', () => {
  ...
  it('should throw when creating a plan for a missing organization', async () => {
    ...
    await expect(service.createSuccessPlan('missing-org', 'Plan')).rejects.toThrow('Organization not found');
  ...
  it('should complete a milestone and publish an event', async () => {
    ...
    expect(messageBus.publish).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        payload: expect.objectContaining({ milestoneId: 'milestone-1', crmOrganizationId: 'org-123' }),
      })
    );
  });

  it('should trigger an intervention through MissionManager and persist it', async () => {
    const result = await service.triggerIntervention('org-123', 'Customer executive check-in', 'high');
  ...
  it('should throw when triggering an intervention for a missing organization', async () => {
    ...
    await expect(service.triggerIntervention('missing-org', 'Call')).rejects.toThrow('Organization not found');

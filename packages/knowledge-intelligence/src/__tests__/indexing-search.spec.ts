...
describe("Knowledge indexing and search", () => {
  ...
  it("should weight title, tags, summary and content tokens", () => {
    ...
    expect((sales as { weight: number }).weight).toBeGreaterThan(
      (negotiation as { weight: number }).weight,
    );
    expect((playbook as { weight: number }).weight).toBeGreaterThan(
      (negotiation as { weight: number }).weight,
    );
  });
  ...
  it("should reindex an article and publish the index event", async () => {
    ...
    expect(
      ctx.events.some((e) => e.type === KnowledgeEventType.INDEX_UPDATED && e.tenantId === orgId),
    ).toBe(true);
  });

  it("should reindex all articles in a tenant", async () => {
    await ctx.tenantContext.runAsync({ tenantId: orgId }, async () => {
      const a1 = await ctx.articleService.createArticle({
        title: "A Guide",
        content: "Guide content A",
        category: "Ops",
        tags: [],
      });
      const a2 = await ctx.articleService.createArticle({
        title: "B Guide",
        content: "Guide content B",
        category: "Sales",
        tags: [],
      });
  ...
  it("should respect article status filtering in search", async () => {
    await ctx.tenantContext.runAsync({ tenantId: orgId }, async () => {
      ...
      const draftResults = await ctx.searchService.search(
        orgId,
        "sales",
        10,

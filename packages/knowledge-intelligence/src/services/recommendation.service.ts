...
export class RecommendationService {
  ...
  async generateRecommendations(health: KnowledgeHealth, organizationId?: string): Promise<KnowledgeRecommendation[]> {
    ...
    if (health.kpis.totalArticles === 0) {
      recommendations.push({
        ...
        reason: 'No articles are on record. Seed the base with onboarding, process and policy documentation.',
        priority: 'high',
      });
  ...
  private addContentGaps(articles: KnowledgeArticle[], recommendations: KnowledgeRecommendation[]): void {
  ...
  private addStaleContent(articles: KnowledgeArticle[], recommendations: KnowledgeRecommendation[]): void {
  ...
  private addIndexCoverage(health: KnowledgeHealth, recommendations: KnowledgeRecommendation[]): void {
  ...
  private addDraftBacklog(health: KnowledgeHealth, recommendations: KnowledgeRecommendation[]): void {
  ...
  private addDuplication(articles: KnowledgeArticle[], recommendations: KnowledgeRecommendation[]): void {
    const seen = new Map<string, string>();
    const duplicates: Array<{ a: string; b: string }> = [];
    for (const article of articles) {
      const normalized = article.title.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();

...
export class FinanceController {
  ...
  sendInvoice(@Param('organizationId') organizationId: string, @Param('id') id: string) {
    return this.tenantContext.run({ tenantId: organizationId }, () => this.invoiceService.sendInvoice(id, organizationId));
  }

  @Post(':organizationId/invoices/:id/pay')
  payInvoice(@Param('organizationId') organizationId: string, @Param('id') id: string) {
    return this.tenantContext.run({ tenantId: organizationId }, () => this.invoiceService.markAsPaid(id, organizationId));
  }

  @Post(':organizationId/invoices/:id/cancel')
  cancelInvoice(@Param('organizationId') organizationId: string, @Param('id') id: string) {
    return this.tenantContext.run({ tenantId: organizationId }, () => this.invoiceService.cancelInvoice(id, organizationId));
  }
}

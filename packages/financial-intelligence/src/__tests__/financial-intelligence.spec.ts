...
describe("FinancialIntelligence", () => {
  ...
  it("should manage the invoice lifecycle and record income on payment", async () => {
    ...
    await ctx.tenantContext.runAsync({ tenantId: orgId }, async () => {
      ...
      const transactions = await ctx.transactionService.listTransactions(orgId, {
        type: TransactionType.INCOME,
      });
  ...
  it("should refuse to update or cancel records owned by another tenant", async () => {
    ...
    await expect(
      ctx.budgetService.updateBudget(budgetA.id, { amount: 9999, organizationId: orgB }),
    ).rejects.toThrow("Budget not found");
    await expect(ctx.invoiceService.markAsPaid(invoiceA.id, orgB)).rejects.toThrow(
      "Invoice not found",
    );
    await expect(ctx.invoiceService.cancelInvoice(invoiceA.id, orgB)).rejects.toThrow(
      "Invoice not found",
    );
  });
});

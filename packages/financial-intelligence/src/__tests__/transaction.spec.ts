...
describe("Transaction service", () => {
  ...
  it("should list and filter transactions by type and status", async () => {
    ...
    const income = await ctx.transactionService.listTransactions(orgId, {
      type: TransactionType.INCOME,
    });
    expect(income).toHaveLength(1);
    expect(income[0].category).toBe("Sales");

    const pending = await ctx.transactionService.listTransactions(orgId, {
      status: TransactionStatus.PENDING,
    });
  ...
  it("should not cancel a transaction owned by another tenant", async () => {
    ...
    await expect(ctx.transactionService.cancelTransaction(transaction.id, orgB)).rejects.toThrow(

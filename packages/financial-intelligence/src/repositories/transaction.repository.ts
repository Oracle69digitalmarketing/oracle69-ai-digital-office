...
export interface TransactionRepository {
  create(transaction: Omit<FinTransaction, 'id' | 'createdAt' | 'updatedAt'>): Promise<FinTransaction>;
  ...
  findByOrganization(organizationId: string, options?: { type?: TransactionType; status?: TransactionStatus }): Promise<FinTransaction[]>;
...
export class PrismaTransactionRepository implements TransactionRepository {
  ...
  async create(transaction: Omit<FinTransaction, 'id' | 'createdAt' | 'updatedAt'>): Promise<FinTransaction> {
    const data = {
      ...transaction,
  ...
  async findByOrganization(organizationId: string, options?: { type?: TransactionType; status?: TransactionStatus }): Promise<FinTransaction[]> {
...
export class InMemoryTransactionRepository implements TransactionRepository {
  ...
  async findByOrganization(organizationId: string, options?: { type?: TransactionType; status?: TransactionStatus }): Promise<FinTransaction[]> {
    return Array.from(this.transactions.values())
      .filter(t => t.organizationId === organizationId && (!options?.type || t.type === options.type) && (!options?.status || t.status === options.status))

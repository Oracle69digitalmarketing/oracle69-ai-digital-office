...
describe('MiReportService', () => {
  ...
  const seo = { organicLeads: 5, organicShare: 0.5, avgPosition: 11, keywordsTracked: 15, metrics: {} };
  const conversion = { totalLeads: 10, conversions: 5, conversionRate: 0.5, bestSource: 'seo', weakestSource: 'paid', sources: [], funnel: {} };
  ...
  beforeEach(() => {
    ...
    service = new MiReportService(campaignEngine, seoEngine, conversionEngine, missionManager, messageBus);
    prisma = (service as any).prisma;
    prisma.miGrowthReport.create = jest.fn().mockResolvedValue({ id: 'report-1', period: 'Q3 2026' });
  ...
  it('should compose a healthy growth report and publish an event without escalation', async () => {
    ...
    expect(report.summary).toEqual(
      expect.objectContaining({ campaign, seo, conversion })
    );
  ...
  it('should escalate a growth mission when there is no inbound lead volume', async () => {
    campaignEngine.compute.mockResolvedValue({
      ...
      channels: campaign.channels.map((c: any) => ({ ...c, leads: 0, conversions: 0, conversionRate: 0, spend: 0, roas: 0 })),
    });
    conversionEngine.compute.mockResolvedValue({ ...conversion, totalLeads: 0, conversions: 0, conversionRate: 0 });

    await service.generateReport('org-1');

    expect(missionManager.createMission).toHaveBeenCalledWith(
      expect.objectContaining({ priority: 'critical', owner: 'marketing-intelligence', status: MissionStatus.DRAFT })
    );
    expect(messageBus.publish).toHaveBeenCalledWith(

...
describe('MiGrowthInsightEngine', () => {
  ...
  const campaign = {
    ...
    channels: [
      { channel: 'seo', leads: 1, conversions: 1, conversionRate: 0.5, revenueAttributed: 2000, spend: 0, roas: 0, cac: 0, costPerLead: 0 },
      { channel: 'paid', leads: 1, conversions: 0, conversionRate: 0, revenueAttributed: 0, spend: 1000, roas: 0.5, cac: 0, costPerLead: 1000 },
    ],
  };
  const seo = { organicLeads: 1, organicShare: 0.5, avgPosition: 11.5, keywordsTracked: 11, metrics: { organicVisits: 120 } };
  const conversion = {
    totalLeads: 2,
    conversions: 1,
    conversionRate: 0.5,
  ...
  beforeEach(() => {
    ...
    engine = new MiGrowthInsightEngine(modelProvider, campaignEngine, seoEngine, conversionEngine, messageBus, memory);
  ...
  it('should generate AI growth insights, persist them, save memory and publish events', async () => {
    modelProvider.analyze.mockResolvedValue({
      content: JSON.stringify({
        insights: [{ type: 'campaign', content: 'Paid channels are under-performing.', confidence: 0.9 }],
        recommendations: [{ title: 'Shift budget', priority: 'high', action: 'Move spend to referral.', expectedImpact: 'Higher ROAS' }],
        contentBriefs: [{ channel: 'email', topic: 'Spring newsletter', targetAudience: 'Prospects', keyPoints: ['Point A', 'Point B'] }],
        pricingSuggestions: [{ product: 'Enterprise Plan', currentPrice: 50000, suggestedPrice: 52500, rationale: 'Value alignment', confidence: 0.8 }],
      }),
    });
    ...
    expect(messageBus.publish).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ type: MarketingIntelligenceEventType.PRICING_SUGGESTION_GENERATED })
    );
  });
  ...
  it('should fall back when the model returns empty arrays', async () => {
    modelProvider.analyze.mockResolvedValue({
      content: JSON.stringify({ insights: [], recommendations: [], contentBriefs: [], pricingSuggestions: [] }),
    });
  ...
  it('should produce deterministic pricing suggestions from won opportunities', async () => {
    modelProvider.analyze.mockRejectedValue(new Error('provider unavailable'));
    prisma.crmOpportunity.findMany = jest.fn().mockResolvedValue([{ name: 'Enterprise Plan', value: 50000 }]);
  ...
  it('should propagate errors from the underlying compute engines', async () => {
    ...
    await expect(engine.generateGrowthInsights('missing-org')).rejects.toThrow('Organization not found');

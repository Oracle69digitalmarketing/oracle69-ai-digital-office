...
export class MiGrowthInsightEngine {
  ...
  async generateGrowthInsights(
    ...
    try {
      ...
      this.logger.warn(
        `AI growth insight generation failed for organization ${organizationId}; using deterministic fallback: ${(error as Error).message}`,
      );
  ...
  private async buildDeterministicResult(
    ...
    if (insights.length === 0) {
      insights.push({
        type: "growth",
        content:
          "Lead generation and conversion are on track with no material growth risks detected.",
        confidence: 0.6,
        source: "deterministic",
      });
    }
    ...
    if (spend > 0 && worstRoas < 1) {
      recommendations.push({
        ...
        priority: "high",
        action:
          "Shift spend from channels returning less than 1x ROAS toward the best-performing sources.",
    ...
    if (totalLeads === 0) {
      recommendations.push({
        ...
        priority: "critical",
        action:
          "Launch campaigns across at least two channels and register them in campaign management.",
  ...
  private async buildDeterministicPricing(
    context: GrowthContext,
    organizationId: string,
  ): Promise<GeneratedPricingSuggestion[]> {
    ...
    for (const opportunity of topOpportunities) {
      suggestions.push({
        ...
        rationale:
          "Maintain current reference pricing while conversion data matures; revisit after the next campaign cycle.",
        confidence: 0.5,
        source: "deterministic",
      });
    }
    ...
    if (
      context.conversion.totalLeads > 0 &&
      context.conversion.conversionRate < 0.1 &&
      ...
      suggestions.push({
        ...
        rationale:
          "Low conversion may indicate a perceived value mismatch; consider a modest price adjustment or added-value bundling.",
        confidence: 0.4,
        source: "deterministic",
      });
    }
  ...
  private async persistAndPublish(
    ...
    if (result.pricingSuggestions.length > 0) {
      this.messageBus.publish(
        ...
        new MarketingIntelligenceEvent(
          MarketingIntelligenceEventType.PRICING_SUGGESTION_GENERATED,
          {
            organizationId,
...
function normalizePricing(
  raw: RawPricing[] | undefined,
  source: "ai" | "deterministic",
): GeneratedPricingSuggestion[] {
  if (!Array.isArray(raw)) return [];
  return raw

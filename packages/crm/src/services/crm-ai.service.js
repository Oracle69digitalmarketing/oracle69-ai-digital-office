...
let CrmAiService = CrmAiService_1 = class CrmAiService {
    messageBus;
    tenantContext;
    logger = new Logger(CrmAiService_1.name);
    prisma = new PrismaClient();
    genAI;
    groqApiKey;
    constructor(messageBus, tenantContext) {
    ...
    `;
        try {
            ...
            this.messageBus.publish(CrmEventType.LEAD_SCORED, new CrmEvent(CrmEventType.LEAD_SCORED, { leadId, score: data.score, rationale: data.rationale }), { tenantId });
            return data;
        }
        catch (error) {
    ...
    async predictOpportunityProbability(opportunityId) {
        const tenantId = this.tenantContext.resolveTenantId();
    ...
    `;
        try {
            ...
            this.messageBus.publish(CrmEventType.OPPORTUNITY_PREDICTED, new CrmEvent(CrmEventType.OPPORTUNITY_PREDICTED, { opportunityId, probability: data.probability, rationale: data.rationale }), { tenantId });
            return data;
        }
        catch (error) {
    ...
    async summarizeActivity(activityId) {
        const tenantId = this.tenantContext.resolveTenantId();
    ...
    `;
        try {
            const text = (await this.generate(prompt)).replace(/```json/g, '').replace(/```/g, '');
            const data = JSON.parse(text);
            this.messageBus.publish(CrmEventType.ACTIVITY_SUMMARIZED, new CrmEvent(CrmEventType.ACTIVITY_SUMMARIZED, { activityId, ...data }), { tenantId });
            return data;
        }
        catch (error) {
...
CrmAiService = CrmAiService_1 = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [MessageBus,

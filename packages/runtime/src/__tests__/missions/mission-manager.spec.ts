...
import { InMemoryMissionRepository, MissionConflictError } from '../../persistence/mission.repository.js';
...
describe('MissionManager', () => {
  ...
  it('should publish tenant-aware mission events through the canonical EventBus', async () => {
    ...
    expect(published).toContainEqual(expect.objectContaining({ type: RuntimeEventType.MISSION_CREATED }));
    expect(published).toContainEqual(expect.objectContaining({ type: RuntimeEventType.MISSION_STARTED }));
  ...
  it('should prevent duplicate mission creation within a tenant', async () => {
    ...
    await expect(manager.createMission(mission({ id: 'm2', missionKey: 'ops-recovery' }))).rejects.toThrow(
  ...
  it('should enforce tenant scope when no tenant is resolvable', async () => {
    ...
    await expect(strictManager.createMission(mission({ tenantId: '' as string }))).rejects.toThrow(TenantContextError);
  });

...
const formedEvent = (options: Record<string, any> = {}): RuntimeEvent =>
  new RuntimeEvent(options.type ?? RuntimeEventType.MISSION_STARTED, options.payload ?? { missionId: 'm1' }, options);
...
describe('InMemoryEventLog', () => {
  ...
  it('should support tenant-scoped queries and filters', async () => {
    const log = new InMemoryEventLog();
    await log.write(formedEvent({ type: RuntimeEventType.MISSION_CREATED, tenantId: 'org-a', missionId: 'm-a', correlationId: 'corr-a' }));
    await log.write(formedEvent({ type: RuntimeEventType.MISSION_STARTED, tenantId: 'org-a', missionId: 'm-a', correlationId: 'corr-a' }));
    await log.write(formedEvent({ type: RuntimeEventType.MISSION_CREATED, tenantId: 'org-b', missionId: 'm-b', correlationId: 'corr-b' }));
...
describe('PrismaEventLog', () => {
  ...
  it('should enforce tenant scoping when listing events', async () => {
    ...
    expect(prisma.runtimeEventLog.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ tenantId: 'org-1', type: RuntimeEventType.MISSION_CREATED }),
      })
    );
  });

...
describe("EiInsightEngine", () => {
  ...
  beforeEach(() => {
    ...
    healthEngine = {
      compute: jest
        .fn()
        .mockResolvedValue({ status: "healthy", score: 80, reasoning: "ok", factors: [] }),
    };
    forecastEngine = {
      compute: jest.fn().mockResolvedValue({ expectedRevenue: 150000, period: "Q3 2026" }),
    };
    messageBus = { publish: jest.fn() };
    memory = { save: jest.fn().mockResolvedValue(undefined) };

    engine = new EiInsightEngine(
      modelProvider,
      kpiEngine,
      healthEngine,
      forecastEngine,
    ...
    modelProvider.analyze.mockResolvedValue({
      content: JSON.stringify({
        insights: [
          {
            type: "forecast",
            content: "Revenue is expected to grow this quarter.",
            confidence: 0.9,
          },
        ],
        recommendations: [
          {
            ...
            priority: "high",
            action: "Launch outbound campaigns.",
            expectedImpact: "+20% pipeline",
          },
        ],
      }),
    });
  ...
  it("should produce a critical retention recommendation when business health is critical", async () => {
    modelProvider.analyze.mockRejectedValue(new Error("provider unavailable"));
    healthEngine.compute.mockResolvedValue({
      status: "critical",
      score: 30,
      reasoning: "bad",
      factors: [],

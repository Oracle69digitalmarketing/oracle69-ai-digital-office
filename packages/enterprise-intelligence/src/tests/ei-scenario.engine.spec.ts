...
describe('EiScenarioEngine', () => {
  ...
  it('should project a revenue uplift from a win-rate improvement', async () => {
    ...
    const result = await engine.runScenario('org-1', { name: 'Improved win rate', winRateDelta: 0.5 });
  ...
  it('should reject out-of-range scenario parameters', async () => {
    await expect(engine.runScenario('org-1', { winRateDelta: 2 })).rejects.toThrow('Invalid scenario parameter');
    await expect(engine.runScenario('org-1', { averageDealValueDelta: -1 })).rejects.toThrow('Invalid scenario parameter');
    await expect(engine.runScenario('org-1', { churnReduction: 1.5 })).rejects.toThrow('Invalid scenario parameter');

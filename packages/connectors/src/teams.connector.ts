...
export class TeamsConnector extends AbstractConnector {
  ...
  async connect(credentials: any): Promise<void> {
    ...
    const credential: TokenCredential = {
      getToken: async () => ({ token: credentials.accessToken, expiresOnTimestamp: Date.now() + 3600000 })
    } as any;

    const authProvider = new TokenCredentialAuthenticationProvider(credential, { scopes: ['Team.ReadBasic.All', 'ChannelMessage.Send'] });
  ...
  private async sendMessage(params: any): Promise<ConnectorResult> {
    ...
    const response = await this.graphClient!.api(`/teams/${teamId}/channels/${channelId}/messages`).post({ body: { content } });
    return { success: true, data: response };
  }

  private async sendRichMessage(params: any): Promise<ConnectorResult> {
    const { teamId, channelId, content, attachment } = params;
    const response = await this.graphClient!.api(`/teams/${teamId}/channels/${channelId}/messages`).post({
  ...
  private async createChannel(params: any): Promise<ConnectorResult> {
    ...
    const response = await this.graphClient!.api(`/teams/${teamId}/channels`).post({ displayName, description });
    return { success: true, data: response };
  }
  ...
  private async mentionUser(params: any): Promise<ConnectorResult> {
    ...
    const response = await this.graphClient!.api(`/teams/${teamId}/channels/${channelId}/messages/${messageId}/replies`).post({
        body: { contentType: 'html', content },
  ...
  private async uploadFile(params: any): Promise<ConnectorResult> {
    ...
    const response = await this.graphClient!.api(`/teams/${teamId}/channels/${channelId}/files/content`).put(content);
    return { success: true, data: response };
  }

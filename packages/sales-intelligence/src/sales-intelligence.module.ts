...
import { RuntimeModule, MemoryManager, MissionManager, MessageBus, TenantContextService } from '@oracle69/runtime';
  ...
  providers: [
    ...
    },
    {
        ...
        inject: [
            ...
            MemoryManager, MissionManager, TenantContextService
        ]
    },
    // Override engines to inject interface implementation
    {
        provide: LeadScoringEngine,
        useFactory: (provider: AiModelProvider, bus: MessageBus, tc: TenantContextService) => new LeadScoringEngine(provider, bus, tc),
        inject: ['AiModelProvider', MessageBus, TenantContextService]
    },
    {
        provide: OpportunityEngine,
        useFactory: (provider: AiModelProvider, bus: MessageBus, tc: TenantContextService) => new OpportunityEngine(provider, bus, tc),
        inject: ['AiModelProvider', MessageBus, TenantContextService]
    },
    {
        provide: DealRiskEngine,
        useFactory: (provider: AiModelProvider, bus: MessageBus, tc: TenantContextService) => new DealRiskEngine(provider, bus, tc),
        inject: ['AiModelProvider', MessageBus, TenantContextService]
    },
    {
        provide: NextBestActionEngine,
        useFactory: (provider: AiModelProvider, bus: MessageBus, tc: TenantContextService) => new NextBestActionEngine(provider, bus, tc),
        inject: ['AiModelProvider', MessageBus, TenantContextService]
    },
    {
        provide: AccountIntelligenceEngine,
        useFactory: (provider: AiModelProvider, bus: MessageBus, tc: TenantContextService) => new AccountIntelligenceEngine(provider, bus, tc),
        inject: ['AiModelProvider', MessageBus, TenantContextService]
    },
    {
        provide: PipelineIntelligenceEngine,
        useFactory: (provider: AiModelProvider, bus: MessageBus, tc: TenantContextService) => new PipelineIntelligenceEngine(provider, bus, tc),
        inject: ['AiModelProvider', MessageBus, TenantContextService]
    },
    {
        provide: ExecutiveIntelligenceEngine,
        useFactory: (provider: AiModelProvider, bus: MessageBus, tc: TenantContextService) => new ExecutiveIntelligenceEngine(provider, bus, tc),
        inject: ['AiModelProvider', MessageBus, TenantContextService]
    },
    {
        provide: RelationshipIntelligenceEngine,
        useFactory: (provider: AiModelProvider, tc: TenantContextService) => new RelationshipIntelligenceEngine(provider, tc),

...
export class HrAiService {
  ...
  async generateInsights(health: HrHealth): Promise<HrAiInsight[]> {
    ...
    try {
      ...
      this.logger.warn(`HR AI insight generation failed; using deterministic fallback: ${(error as Error).message}`);
      return this.deterministicInsights(health);
    }
  }
  ...
  private deterministicInsights(health: HrHealth): HrAiInsight[] {
    ...
    if (kpis.headcount === 0) {
      insights.push({
        ...
        content: 'No active employees are on record. Prioritize hiring to establish the operational workforce.',
        priority: 'high',
    ...
    if (kpis.openPositions > 0) {
      ...
      insights.push({
        ...
        content: 'All positions are filled. Plan headcount against upcoming business needs to avoid hiring lag.',
        priority: 'low',
        impact: 'Readiness',
      });
    }
...
function normalizeInsights(raw: RawInsight[] | undefined): HrAiInsight[] {
  ...
  return raw
    ...
    .map((item) => ({
      ...
      title: typeof item.title === 'string' && item.title.length > 0 ? item.title : 'HR recommendation',
      content: item.content as string,
      priority: item.priority === 'low' || item.priority === 'high' ? item.priority : 'normal',
      impact: typeof item.impact === 'string' && item.impact.length > 0 ? item.impact : 'Improved workforce health',
    }));
}

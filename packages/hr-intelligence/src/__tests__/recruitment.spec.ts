...
describe('Recruitment', () => {
  ...
  it('should not create a candidate for a position of another tenant', async () => {
    ...
    await expect(
      ctx.tenantContext.runAsync({ tenantId: attacker }, () =>
        ctx.recruitmentService.createCandidate({ positionId: position.id, name: 'X', email: 'x@example.com' }),
      ),
  ...
  it('should enforce tenant isolation for candidates', async () => {
    ...
    await expect(ctx.recruitmentService.hireCandidate(candidate.id, attacker)).rejects.toThrow('Candidate not found');
  });
});

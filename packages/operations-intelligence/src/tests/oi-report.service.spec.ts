...
describe("OiReportService", () => {
  ...
  beforeEach(() => {
    ...
    prisma.oiOperationsReport.create = jest
      .fn()
  ...
  it("should compose a healthy operations report and publish an event without escalation", async () => {
    ...
    expect(report.summary).toEqual(expect.objectContaining({ operations, workflows, agents }));
  ...
  it("should escalate an operations mission when operational health is critical", async () => {
    ...
    agentEngine.compute.mockResolvedValue({
      ...
      utilizationAvg: 0,
      idleAgents: 0,
      metrics: { ...agents.metrics, totalAgents: 0 },
    });

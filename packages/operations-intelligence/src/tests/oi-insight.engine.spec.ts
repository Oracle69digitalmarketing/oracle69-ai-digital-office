...
describe('OiInsightEngine', () => {
  ...
  const agents = {
    period: 'Q3 2026',
    agents: [
      { agentId: 'agent-1', agentName: 'A', tasksAssigned: 3, tasksCompleted: 1, completionRate: 0.33, utilizationRate: 0.67, avgExecutionTime: 20, status: 'active' },
      { agentId: 'agent-2', agentName: 'B', tasksAssigned: 2, tasksCompleted: 0, completionRate: 0, utilizationRate: 0, avgExecutionTime: null, status: 'idle' },
    ],
  ...
  beforeEach(() => {
    ...
    engine = new OiInsightEngine(modelProvider, operationsEngine, workflowEngine, agentEngine, messageBus, memory);
  ...
  it('should generate AI operational insights, persist them, save memory and publish events', async () => {
    modelProvider.analyze.mockResolvedValue({
      content: JSON.stringify({
        insights: [{ type: 'tasks', content: 'Backlog is growing faster than completion.', confidence: 0.9 }],
        recommendations: [{ title: 'Rebalance backlog', priority: 'high', action: 'Dispatch oldest tasks.', expectedImpact: 'Faster cycle time' }],
      }),
    });
  ...
  it('should produce a neutral deterministic result when operations are healthy', async () => {
    ...
    agentEngine.compute.mockResolvedValue({
      ...agents,
      agents: [
        { agentId: 'agent-1', agentName: 'A', tasksAssigned: 3, tasksCompleted: 3, completionRate: 1, utilizationRate: 1, avgExecutionTime: 20, status: 'active' },
      ],
    });

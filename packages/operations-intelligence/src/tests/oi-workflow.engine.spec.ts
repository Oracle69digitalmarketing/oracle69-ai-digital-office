...
describe("OiWorkflowEngine", () => {
  ...
  it("should compute workflow health metrics and persist a snapshot", async () => {
    prisma.workflow.findMany = jest
      .fn()
      .mockResolvedValue([activeWorkflow, completedWorkflow, failedWorkflow]);
  ...
  it("should flag started-but-stale workflows as stalled", async () => {
    prisma.workflow.findMany = jest
      .fn()
      .mockResolvedValue([{ ...activeWorkflow, createdAt: new Date(Date.now() - 10 * 86400000) }]);
